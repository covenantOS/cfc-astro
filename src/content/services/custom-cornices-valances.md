---
title: "Cornices & Valances in St. Petersburg, FL"
description: "Custom cornices & valances in St. Petersburg, built to your exact window dimensions. Fabric valances, box cornices & coordinated top treatments. Free."
h1: "Custom Cornices & Valances in St. Petersburg"
slug: custom-cornices-valances
type: service
---
Custom cornices and valances in Tampa Bay? Custom Fabric Creations designs and installs tailor-made window top treatments that finish the look of any room. Free in-home consultation.

## What Are Cornices and Valances — And What's the Difference

These two terms get used interchangeably, but they're different:

Cornices are rigid, box-style top treatments — typically constructed from wood or MDF with a decorative facing and often covered in fabric. They mount directly above the window and create a strong architectural line. Cornices work particularly well in rooms with traditional or transitional aesthetics, or in spaces where you want a deliberate, structured finish to the window.

Valances are softer, fabric-based top treatments that hang or drape across the top of the window. They can be straight, shaped, or swagged, and they tend to feel lighter and less formal than cornices. Valances are a good choice when you want the finishing touch without the full architectural weight of a cornice — or when you want to add a fabric element that coordinates with drapery panels below.

Both are custom — and both solve the same core problem: the top of the window needs to look finished, not like an afterthought.




![Top](/images/services/custom-cornices-valances/top.webp)

## Why Tampa Bay Windows Often Need Custom Top Treatments

Florida homes have a window problem that doesn't exist in most other markets.

Concrete block construction, vaulted ceilings, off-standard window heights, and hurricane-impact windows all create situations where standard top treatments don't fit. The window sits higher than the trim. The mounting bracket for the blinds doesn't have anywhere clean to attach. The ceiling angle creates a gap that retail top treatments can't bridge.

Custom cornices and valances solve this because they're built to your exact measurements and installed to work with your specific window — not adapted around it.

Additionally, the humidity and temperature swings in Tampa Bay affect the materials used in top treatments. A poorly constructed cornice can sag over time. We build with materials and construction methods that hold up to Florida conditions.

## Types of Custom Top Treatments We Build in Tampa Bay

Cornices — Box-style rigid top treatments available in a range of heights and projections. We build them in wood or MDF with fabric facing, and offer different facing profiles depending on the look you want. Cornices are mounted directly to the wall above the window, creating a clean architectural line that works in both traditional and contemporary spaces.

Fabric valances — Draped or shaped fabric treatments mounted on a board or batten. Available in a range of styles: straight edge, patterned, shaped, or with returns that wrap around to the wall. Fabric valances coordinate naturally with drapery panels below and add softness to the top of the window without the full weight of a cornice.

Swagged and shaped valances — More decorative valance styles with cascading fabric or geometric shapes. These work best in formal living rooms, dining rooms, or spaces where the window treatment is a focal point of the room's design.

Coordinated drapery top treatments — When we design a full drapery system, we typically design the top treatment as part of the overall specification — matching the fabric, the heading style, and the proportions so everything reads as one intentional design decision.




![Bottom](/images/services/custom-cornices-valances/bottom.webp)

## How We Work — From Consultation to Installation

- Free in-home consultation. We come to your home with fabric samples, wood and MDF samples, and photographs of previous installations. We look at your windows, discuss the look you're going for, and show you what the options look like in your actual room — not a showroom.
- Measurement and design. We measure every window individually. For cornices, we account for the wall surface above the window, the mounting conditions, and how the cornice will relate to any other window treatments below it. For valances, we determine the appropriate length, projection, and mounting style.
- Custom manufacturing. Every cornice and valance is built to your specific measurements in our workroom. We select materials appropriate for the look and the conditions, cut and construct the frame, and finish with your chosen fabric.
- Professional installation. We install every top treatment we build. We confirm the fit, the level, and the relationship to the window and the wall. A cornice that's not installed level looks wrong — we don't leave until it's right.

## Serving the Greater Tampa Bay Area

We design and install custom cornices and valances for homeowners across Tampa Bay:

Hillsborough County: Tampa, Brandon, Riverview, Apollo Beach, Lutz, Odessa

Pinellas County: St. Petersburg, Clearwater, Tarpon Springs, Palm Harbor

Pasco County: Wesley Chapel, Zephyrhills, Land O' Lakes, Trinity

Sarasota / Manatee Counties: Sarasota, Venice, Bradenton, Palmetto

Not in one of these areas? Call us — if we can help, we will.

## Ready to finish the look of your windows?

Call (727) 240-4512 or request your free in-home consultation.

We're available by appointment — evenings and weekends included.

## Frequently Asked Questions

### What's the difference between a cornice and a valance?

A cornice is a rigid, box-style top treatment — think of it as a finished frame at the top of the window. A valance is softer and made from fabric, with more variety in shape and drape. Both are top treatments; the difference is material and visual weight. Which is right for your room depends on the look you want and whether you have drapery panels below.

### Can you add a cornice or valance to existing blinds or shades?

Yes — in many cases, we can add a top treatment to an existing window treatment installation. The key is whether the existing hardware can accommodate it and whether the proportions work. We assess this at the consultation.

### What fabric options are available for cornices and valances?

We work with a wide range of upholstery-grade fabrics appropriate for top treatments. We also offer contrasting or complementary fabrics for multi-layer treatments. At the consultation, we show you what's available and discuss which materials work best for the look and the conditions in your home.

### Do cornices and valances work with all window types?

They work with most — but they require careful measurement and design for non-standard windows. Florida's concrete block construction, vaulted ceilings, and hurricane-impact window systems can create mounting challenges that require custom solutions. That's exactly what we do.

### How long does a custom cornice or valance take?

From the initial consultation to installation, most top treatments take 3 to 5 weeks, depending on material availability and the complexity of the design. We give you a specific timeline at the estimate.
